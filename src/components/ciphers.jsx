...
class Cipher extends React.Component{
    ...
    plainToCipher=(event)=>{
        ...
        for(var i=0;i<plainText.length;i++){
            ...
            answerCharCode+=String.fromCharCode(parseInt((((plainText[i].charCodeAt() + key[keyLength].charCodeAt())%26)+65)))
    ...
    cipherToPlain=(event)=>{
       ...
       cipherText=cipherText.toUpperCase()
        ...
        for(var i=0;i<cipherText.length;i++){
            ...
            answerCharCode+=String.fromCharCode(parseInt((((cipherText[i].charCodeAt() - key[keyLength].charCodeAt())%26)+65)))
    ...
    plainToShiftCipher=(event)=>{
        ...
        for(var i=0;i<plainText.length;i++){
            answerCharCode+=String.fromCharCode(parseInt((((plainText[i].charCodeAt() - 65 + key)%26)+65)))
    ...
    shiftCipherToPlain=(event)=>{
        ...
        for(var i=0;i<cipherText.length;i++){
            answerCharCode+=String.fromCharCode(parseInt((((cipherText[i].charCodeAt() - 65 + key)%26)+65)))
    ...
    substitutionCipher=(event)=>{
        ...
        var Cipher = {};
        Cipher=plainText.split('').filter(function(v) {
    ...
    substitutionDecipher=(event)=>{
        ...
        Plain=cipherText.split('').filter(function(v) {
    ...
    display=(selectedOption)=>{
        if(selectedOption==='Vigenere'){
            ...
            <div>
            <input type="text" placeholder={`Enter your ${this.state.selectedMode}`} onChange={this.state.selectedMode==='plaintext'?this.plainToCipher:this.cipherToPlain}/>
            
            <input type="text" placeholder="Enter your key"  onChange={this.handleKeyChange}/>
            </div>
            </div>
            )
        }
            ...
            <label className="radio-inline">
                ...
                <div>
                <input type="text" placeholder={`Enter your ${this.state.selectedMode}`} onChange={this.state.selectedMode==='plaintext'?this.plainToShiftCipher:this.shiftCipherToPlain}/>
            
            <input type="number" placeholder="Enter your key"  onChange={this.handleShiftKeyChange}/>
                </div>
                </div>
    ...
    render(){
        ...
        return(
            ...
            <h3>Converted:{this.state.plainText}</h3>
            </div>
        )
    }
}
